...
All notable changes to Snacode are documented here.
...
- **Sidebar brand lockup redesign** — The official pi canvas logo now uses a cropped
  bounding box (no empty board space), displays the Snacode wordmark in Plantin serif,
...
- **Built-in Browser Preview**
  ...
  - Fullscreen mode and PC/mobile/tablet viewport presets for quickly checking web pages without leaving Snacode
...
- **Homepage link**: Added Snacode website button in bottom-left sidebar
...
### Added
- Claude session import from the project context menu, converting local Claude JSONL sessions into Snacode history sessions.
...
### Added
- LAN web service: Settings can now start a local HTTP service so devices on the same network can open Snacode through the host machine's IP and configured port.
...
- Git branch creation: the branch selector can create a new branch from the current branch without leaving Snacode.
...
- Full-screen Settings, Config, and Feedback pages now fit the custom Electron titlebar better and avoid overlapping the Snacode titlebar/brand area.
...
- Settings dropdowns now use a custom Snacode-styled select component instead of native browser select popups.
...
- Snacode branding, fonts, logo treatment, image preview overlays, picker palettes, and terminal typography have been refined for a more consistent desktop feel.
...
- Settings and Config pages no longer overlap the custom titlebar Snacode label when opened in the custom titlebar layout.
...
- In-app update check: Snacode now periodically checks the latest GitHub Release and shows release notes plus browser download links when a newer version is available.
...
### Fixed
- Fixed project avatars for hidden folders such as `.pi` and `.snacode` by ignoring leading dots and whitespace.
