...
const buildDir = mkdtempSync(join(tmpdir(), "Snacode-git-service-build-"));
const repositoryDir = mkdtempSync(join(tmpdir(), "Snacode-git-diff-"));
let GitService;
...
before(() => {
  ...
  git("init");
  git("config", "user.name", "Snacode Test");
...
describe("GitService committed-file diff integration", () => {
  ...
  test("stages resolved merge conflicts and preserves the Git drawer diff baseline", async () => {
    ...
    const mainBranch = git("branch", "--show-current");
    git("checkout", "-b", "Snacode-conflict-test");
    ...
    git("commit", "-m", "ours");
    try {
      git("merge", "Snacode-conflict-test");
    ...
    git("merge", "--abort");
    git("branch", "-D", "Snacode-conflict-test");
  });

  test("does not follow workspace symlinks outside the project", async (context) => {
    const service = new GitService();
    const externalDir = mkdtempSync(join(tmpdir(), "Snacode-external-"));
  ...
  test("unstages files in an unborn repository without deleting the worktree file", async () => {
    const service = new GitService();
    const unbornDir = mkdtempSync(join(tmpdir(), "Snacode-unborn-"));
