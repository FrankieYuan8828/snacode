...
test("builds Feishu model picker card grouped by provider", () => {
	...
	assert.deepEqual(JSON.parse(JSON.stringify(actions[2].value)), {
		action: "Snacode.set_model",
		provider: "anthropic",
		modelId: "claude-sonnet-4",
	});
});

test("parses only Snacode model switch card actions", () => {
	const { parseModelActionValue } = loadModelCardModule();

	assert.deepEqual(JSON.parse(JSON.stringify(parseModelActionValue({ action: "Snacode.set_model", provider: "openai", modelId: "gpt-4o" }))), {
		provider: "openai",
		modelId: "gpt-4o",
	});
	assert.equal(parseModelActionValue({ action: "other", provider: "openai", modelId: "gpt-4o" }), undefined);
	assert.equal(parseModelActionValue({ action: "Snacode.set_model", provider: "", modelId: "gpt-4o" }), undefined);
});
