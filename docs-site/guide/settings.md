...
Snacode 提供图形化配置入口，减少频繁查找和编辑 sd 配置文件的成本。
...
应用启动时会自动检测系统中的 `sd` 命令。自动检测失败时，可以在设置中手动输入路径。

Windows 下支持常见路径形式：

```text
"C:\Program Files\sd\sd.cmd"
C:\\Program Files\\sd\\sd.cmd
C:\Program Files\sd\sd
```
