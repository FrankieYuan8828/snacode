...
站点已使用自定义域名 `https://Snacode.caoayu.top`，Pages workflow 中设置：

```yaml
env:
  VITEPRESS_BASE: /
  DOCS_SITE_ORIGIN: https://Snacode.caoayu.top
```

`docs-site/public/CNAME` 写入 `Snacode.caoayu.top`，构建后进入产物，避免部署冲掉域名绑定。
若临时需要兼容旧地址 `https://ayuayue.github.io/Snacode/`，可本地设置 `VITEPRESS_BASE=/Snacode/`。
