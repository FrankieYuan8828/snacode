 ...
 *
 * 相比 snacode-plan-mode（从 LLM 输出的 Plan: 文本解析 todo），本扩展让 LLM 显式
 ...
 * 是否含 "snacode-todo"）。若被第三方覆盖则停掉 widget 与状态重建，/todo 命令
 * 转而引导用户使用第三方扩展的命令。
 *
...
// widget key 与 appendEntry 的 customType，统一用 snacode-todo 前缀，避免与 plan-mode 冲突
const WIDGET_KEY = "snacode-todo";
const ENTRY_TYPE = "snacode-todo";
// 自身来源标识：sourceInfo.path / source 含此串说明当前生效的 todo 工具仍是本扩展注册的
const SELF_MARKER = "snacode-todo";
...
export default function (pi: ExtensionAPI) {
	...
	/**
	 * 从会话 entries 重建状态：取最后一条 snacode-todo 快照。
