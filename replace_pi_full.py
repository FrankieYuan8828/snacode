...
replacements = {
    # Class names (already done but included for completeness)
    'PiLocator': 'SdLocator',
    'PiProcess': 'SdProcess',
    ...
    'settings:test-pi-proxy': 'settings:test-sd-proxy',
    ...
    'piRpcClient': 'sdRpcClient',
    'piProxyTester': 'sdProxyTester',
    'piUpdateCheck': 'sdUpdateCheck',
    'piUpdate': 'sdUpdate',
    'piExecInstall': 'sdExecInstall',
    'piCheckNpm': 'sdCheckNpm',
    'piCheckCustom': 'sdCheckCustom',
    'piCheck': 'sdCheck',
    'piVersion': 'sdVersion',
    ...
    # npm package names
    '@earendil-works/pi': '@snacode/sd',
    '@earendil-works/pi-coding-agent': '@snacode/sd-coding-agent',
    
    # Repository URLs
    'ayuayue/pi-desktop': 'EasonYuan/snacode',
}

# File patterns to process
file_patterns = ['.ts', '.tsx', '.md', '.json', '.css', '.html']
...
def process_file(filepath):
    try:
        ...
        # Special case: replace pi references in comments and text
        # Be careful with this - only replace standalone 'pi' words
        content = re.sub(r'\b([Pp])i\b', lambda m: m.group(1) + 'd', content)
