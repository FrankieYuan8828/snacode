...
const api = {
	...
	projectResources: {
		...
		createSkill: (input: CreateProjectSkillInput) =>
			ipcRenderer.invoke(ipcChannels.projectResourcesCreateSkill, input) as Promise<SkillSummary>,
		...
		toggleSkill: (projectId: string, skillPath: string, enabled: boolean) =>
			ipcRenderer.invoke(ipcChannels.projectResourcesToggleSkill, projectId, skillPath, enabled) as Promise<SkillSummary>,
		renameSkill: (projectId: string, skillPath: string, newName: string) =>
			ipcRenderer.invoke(ipcChannels.projectResourcesRenameSkill, projectId, skillPath, newName) as Promise<SkillSummary>,
	},
	files: {
		list: (projectId: string) =>
	...
	skills: {
		list: () =>
			ipcRenderer.invoke(ipcChannels.skillsList) as Promise<SkillListResult>,
		create: (input: CreateSkillInput) =>
			ipcRenderer.invoke(ipcChannels.skillsCreate, input) as Promise<SkillSummary>,
		...
		rename: (skillPath: string, newName: string) =>
			ipcRenderer.invoke(ipcChannels.skillsRename, skillPath, newName) as Promise<SkillSummary>,
	},
	prompts: {
		list: () =>
			ipcRenderer.invoke(ipcChannels.promptsList) as Promise<PromptTemplateListResult>,
		create: (input: CreatePromptTemplateInput) =>
			ipcRenderer.invoke(ipcChannels.promptsCreate, input) as Promise<PromptTemplateSummary>,
		delete: (filePath: string) =>
		...
		listByProject: (projectPath: string) =>
			ipcRenderer.invoke(ipcChannels.promptsListByProject, projectPath) as Promise<PromptTemplateListResult>,
		createInProject: (projectPath: string, input: CreatePromptTemplateInput) =>
			ipcRenderer.invoke(ipcChannels.promptsCreateInProject, projectPath, input) as Promise<PromptTemplateSummary>,
		...
		rename: (oldName: string, newName: string) =>
			ipcRenderer.invoke(ipcChannels.promptsRename, oldName, newName) as Promise<PromptTemplateSummary>,
		renameInProject: (projectPath: string, oldName: string, newName: string) =>
			ipcRenderer.invoke(ipcChannels.promptsRenameInProject, projectPath, oldName, newName) as Promise<PromptTemplateSummary>,
	},
	promptStore: {
		...
		import: (data: { title: string; description: string; content: string }) =>
			ipcRenderer.invoke(ipcChannels.promptStoreImport, data) as Promise<PromptTemplateSummary>,
	},
	skillStore: {
		search: (query: string) =>
		...
		import: (item: PromptStoreItem, locationId?: string) =>
			ipcRenderer.invoke(ipcChannels.skillStoreImport, item, locationId) as Promise<SkillSummary>,
	},
	skillHub: {
	...
	yaoPrompts: {
		...
		import: (slug: string, category: string) =>
			ipcRenderer.invoke(ipcChannels.yaoPromptsImport, slug, category) as Promise<PromptTemplateSummary>,
	},
	extensions: {
		// forceRefresh=true 时跳过主进程缓存并补充 npm 版本信息。
		list: (forceRefresh = false) =>
			ipcRenderer.invoke(ipcChannels.extensionsList, forceRefresh) as Promise<ExtensionListResult>,
		...
		update: () =>
			ipcRenderer.invoke(ipcChannels.extensionsUpdate) as Promise<CliUpdateResult>,
	},
	githubSearch: {
		search: (query: string, sortBy: "stars" | "updated" | "forks") =>
			ipcRenderer.invoke(ipcChannels.githubSearch, query, sortBy) as Promise<ExtensionPackageInfo[]>,
	},
	settings: {
		get: () =>
	...
	agents: {
		...
		commands: (agentId: string) =>
			ipcRenderer.invoke("agents:commands", agentId) as Promise<AgentCommand[]>,
	...
	dialog: {
		...
		pickImages: (options?: { title?: string }) =>
			ipcRenderer.invoke(ipcChannels.dialogPickImages, options) as Promise<(ImageContent & { fileName: string })[]>,
	},
...
try {
	...
	console.error("[Snacode preload] Failed to expose desktop API", detail);
	ipcRenderer.send(ipcChannels.preloadError, detail);
}

export type SnacodeDesktopApi = typeof api;
