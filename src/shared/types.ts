...
export type AppSettings = {
	...
	/** 等宽字体预设，commit-mono 为内置 SnacodeCommitMono；custom 时使用 fontFamilyMonoCustom */
...
/** 安装命令执行结果 */
export type AgentInstallExecResult = {
	success: boolean;
...
export type CreateSkillInput = {
	name: string;
	description: string;
	locationId: SkillLocation["id"];
};

/** Prompt Template，对应 ~/.sd/agent/prompts/<name>.md */
export type PromptTemplateSummary = {
	...
	content: string;
	userCreated: boolean;
	/** 模板范围：global (~/.sd/agent/prompts/) 或 project (.sd/prompts/) */
...
export type ExtensionPackageInfo = {
	name: string;
	description: string;
	installCmd: string;
	tags: string[];
