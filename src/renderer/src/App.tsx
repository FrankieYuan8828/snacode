...
function saveSessionSourceFilter(filter: Record<string, Set<"pi" | "codex" | "claude" | "opencode"> | null>) {
	try {
		...
		localStorage.setItem("snacode-session-source-filter", JSON.stringify(obj));
	} catch {
		// 静默失败
	}
}
...
export function App() {
  ...
  /** 输入框发送模式：normal 直接交给 agent，plan 通过隐藏标记触发 Snacode Plan Mode 扩展。 */
  ...
  /** 安装执行结果 */
  const [installResult, setInstallResult] = useState<AgentInstallExecResult | null>(null);
  /** 安装是否已成功完成 */
  ...
  useEffect(() => {
    ...
    const offUiRequest = api.agents.onUiRequest((request) => {
      ...
      if (request.method === "setWidget") {
        ...
        // ref: https://github.com/ayuayue/Snacode/issues/73
  ...
  async function checkNpm() {
    setNpmChecking(true);
    try {
      const result = await api.agentCli.checkNpm();
  ...
  async function execInstallCommand() {
    ...
    try {
      const result = await api.agentCli.execInstall(cmd);
  ...
  async function handleAttachImages() {
    try {
      ...
      setAttachedImages((current) => [...current, ...imageContents]);
    } catch {
      // 用户取消或出错时不作处理
    }
  }
                ...
                {!isCollapsed && project.worktreeEnabled && (
                  <div className="worktree-children worktree-sandbox-list">
                    ...
                    {(() => {
                      ...
                      // Snacode 创建的 worktree 分支使用 pideck/{slug} 命名；侧栏只展示 slug，
          ...
          <div className="toolbar-actions sidebar-bottom-actions">
            <div className="sidebar-bottom-primary-actions">
              ...
              <button
                ...
                onClick={() => api.app.openExternal("https://ayuayue.github.io/Snacode/")}
              >
                <Globe size={17} />
              </button>
            </div>
        ...
        <header ref={chatHeaderRef} className="chat-header">
          <div className="chat-title-block">
            <div className="chat-title-row">
              <strong
                title={activeAgent?.title ?? activeProject?.name ?? "Snacode"}
              >
                {activeAgent?.title ??
