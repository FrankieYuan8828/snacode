...
import type { ChatMessage, FileTreeNode, AgentCommand } from "../../../../shared/types";
...
function isBuiltinDesktopCommand(command: AgentCommand) {
	return command.source == null || command.source === "builtin";
}

function isVisibleDesktopCommand(command: AgentCommand) {
	return !(
...
function getBuiltinCommands(): AgentCommand[] {
	return [
...
export function mergeCommands(commands: AgentCommand[]) {
