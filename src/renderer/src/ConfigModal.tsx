...
function ConfigModalContent(props: ConfigModalProps) {
	...
	const [skillsData, setSkillsData] = useState<SkillListResult>({
		locations: [],
		skills: [],
	});
	const [extensionsData, setExtensionsData] = useState<ExtensionListResult>({
		extensions: [],
		raw: "",
	});
	...
	const [promptsData, setPromptsData] = useState<PromptTemplateListResult>({
		templates: [],
		globalDir: "",
	});
	...
	const [editingPrompt, setEditingPrompt] = useState<PromptTemplateSummary | null>(null);
	...
	const [uninstallExtensionConfirm, setUninstallExtensionConfirm] = useState<ExtensionSummary | null>(null);
	...
	const handleExport = async () => {
		try {
			...
			a.href = url;
			// 文件名含时间戳，便于用户区分多次备份
			a.download = `Snacode-config-${new Date().toISOString().slice(0, 10)}.json`;
			a.click();
	...
	const confirmDeletePrompt = async (target: PromptTemplateSummary) => {
		setError(null);
	...
	const handleEditPrompt = async (template: PromptTemplateSummary) => {
	...
	const handleRenameGlobalSkill = async (skill: SkillSummary, newName: string) => {
		setError(null);
		try {
	...
	const handleEditGlobalSkill = async (skill: SkillSummary) => {
