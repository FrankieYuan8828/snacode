...
export function PromptsTab(props: {
	data: PromptTemplateListResult;
	loading: boolean;
	creating: boolean;
	newName: string;
	newDescription: string;
	/** 当前正在编辑的模板，null 表示未打开编辑器 */
	editingTemplate: PromptTemplateSummary | null;
	/** 编辑器内容 */
	editContent: string;
	/** 编辑器是否正在加载 */
	editLoading: boolean;
	...
	onCreate: () => void;
	onDelete: (template: PromptTemplateSummary) => void;
	onEdit: (template: PromptTemplateSummary) => void;
	onRename: (template: PromptTemplateSummary, newName: string) => Promise<void>;
	onCancelEdit: () => void;
	onQuickSave: () => void;
