...
type WebState = {
	projects: Awaited<ReturnType<SnacodeDesktopApi["projects"]["list"]>>;
	agents: AgentTab[];
...
export function createBrowserApi(): SnacodeDesktopApi {
	return {
		...base,
		projects: {
			...base.projects,
		...
		sessions: {
			...
			list: async (projectId) => {
				if (!projectId) return [];
				const result = await request<{ sessions: Awaited<ReturnType<SnacodeDesktopApi["sessions"]["list"]>> }>(
		...
		agents: {
			...
			runtimeState: async (agentId) => {
				const result = await request<{ state: Awaited<ReturnType<SnacodeDesktopApi["agents"]["runtimeState"]>> }>(
					`/api/agents/${encodeURIComponent(agentId)}/runtime`,
				);
				return result.state;
			},
			cycleModel: async (agentId) => {
				const result = await request<{ state: Awaited<ReturnType<SnacodeDesktopApi["agents"]["cycleModel"]>> }>(
			...
			availableModels: async (agentId) => {
				const result = await request<{ models: Awaited<ReturnType<SnacodeDesktopApi["agents"]["availableModels"]>> }>(
					`/api/agents/${encodeURIComponent(agentId)}/models`,
				);
				return result.models;
			},
			setModel: async (agentId, provider, modelId) => {
				const result = await request<{ state: Awaited<ReturnType<SnacodeDesktopApi["agents"]["setModel"]>> }>(
			...
			refreshModels: async (agentId) => {
				const result = await request<{ state: Awaited<ReturnType<SnacodeDesktopApi["agents"]["refreshModels"]>> }>(
			...
			cycleThinking: async (agentId) => {
				const result = await request<{ state: Awaited<ReturnType<SnacodeDesktopApi["agents"]["cycleThinking"]>> }>(
			...
			setThinking: async (agentId, level) => {
				const result = await request<{ state: Awaited<ReturnType<SnacodeDesktopApi["agents"]["setThinking"]>> }>(
