...
const BUILT_IN_EXTENSIONS = [
	"snacode-ask-question.ts",
	"snacode-nul-redirect-fix.ts",
	"snacode-plan-mode.ts",
	"snacode-todo.ts",
] as const;
...
export class ExtensionManager {
	...
	private listInflight: Promise<ExtensionListResult> | null = null;
	/** 进行中请求是否为强制刷新（含版本信息）。 */
	private listInflightForce = false;

	constructor(
		private readonly locator: SdLocator,
	...
	private async loadList(includeVersionInfo: boolean): Promise<ExtensionListResult> {
	...
	private async scanLocalExtensions(): Promise<ExtensionSummary[]> {
		const extensionsDir = join(this.homeDir, ".snacode", SD_MCP_LIBRARY_DIR);
		...
		for (const entry of entries) {
			...
			const isBuiltIn = name.startsWith("snacode-");
			result.push({
				id: `local:${source}`,
				source,
				path: extensionsDir,
	...
	async uninstall(source: string, scope: ExtensionSummary["scope"] = "user"): Promise<void> {
		const normalized = source.trim();
		if (!normalized) throw new Error("扩展来源不能为空");
		// 阻止卸载 Snacode 内置扩展（如 snacode-file-capture）
		if (source.startsWith("snacode-")) {
			throw new Error("Snacode 内置扩展不可卸载");
		}
		await this.runPi([
			"remove",
			normalized,
	...
	private toUpdateResult(command: string, output: string, updated: boolean): CliUpdateResult {
	...
	private parseListOutput(raw: string): ExtensionSummary[] {
		const result: ExtensionSummary[] = [];
		let scope: ExtensionSummary["scope"] = "unknown";
		let pending: ExtensionSummary | null = null;
