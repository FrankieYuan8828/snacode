...
import { extractMessageText, extractThinkingRaw } from "../agent/messageContent";
...
export class SessionScanner {
   ...
   * 默认 ~/.sd/agent/sessions，加上 settings 中的 sessionDir（如项目 .sd/sessions）。
   * 供子会话父路径推断作为边界。
   */
  ...
  private async resolveConfiguredSessionDir(
    ...
    const globalRaw = this.wslConfig
      ? await this.readSessionDirSettingWsl(`${this.wslConfig.home}/.sd/agent/settings.json`)
      : await this.readSessionDirSettingLocal(join(app.getPath("home"), ".sd", "agent", "settings.json"));
  ...
  private inferProjectPathFromFile(filePath: string) {
    ...
    // 默认布局：~/.sd/agent/sessions/<encoded-cwd>/...
    const marker = "/.sd/agent/sessions/";
