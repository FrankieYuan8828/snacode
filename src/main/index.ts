...
import { githubExtensionService } from "./extensions/GithubExtensionService";
...
async function syncWslEnvironment(settings: AppSettings): Promise<WslEnvironment | null> {
	const environment = settings.wslEnabled && settings.wslDistro && settings.wslUser
		? await resolveWslEnvironment(settings.wslDistro, settings.wslUser, {
			warn: (message, detail) => {
				console.warn(`[Snacode] ${message}`, detail);
...
const RELEASES_URL = "https://github.com/ayuayue/Snacode/releases";
const LATEST_RELEASE_API =
	"https://api.github.com/repos/ayuayue/Snacode/releases/latest";
...
async function checkForAppUpdate(
	...
	const response = await fetch(LATEST_RELEASE_API, {
		headers: {
			...
			"User-Agent": `snacode/${currentVersion}`,
		},
	});
	if (!response.ok) {
...
async function downloadUpdateAsset(asset: AppUpdateAsset): Promise<AppUpdateDownloadResult> {
	...
	return new Promise((resolve, reject) => {
		...
		request.setHeader("User-Agent", `snacode/${app.getVersion()}`);
...
function printStartupInfo() {
	...
	mainWindow.webContents.executeJavaScript(`
		...
		console.log("%c  https://github.com/ayuayue/Snacode/issues", "color: #3b82f6; text-decoration: underline;");
		console.log("");
...
function registerIpc() {
	...
	ipcMain.handle(ipcChannels.dialogPickImages, async (_event, options?: { title?: string }) => {
		const result = await dialog.showOpenDialog({
		...
		const images = await Promise.all(
			result.filePaths.map(async (filePath) => {
				...
				const mimeType = mimeTypeMap[ext] || "image/octet-stream";
				return {
	...
	ipcMain.handle(
		...
		async (_event, projectId: string, worktreePath: string) => {
			...
			// 也要清理 Snacode 项目记录，否则重启后会从 projects.json 恢复成"删不掉"。
			if (ok || !stillInGit) {
	...
	async function ensureGenProcess(
		...
		genRpcClient = new SdRpcClient(genProcess.stdin!, genProcess.stdout!);
	...
	async function quickGenerate(projectPath: string, prompt: string): Promise<string> {
		...
		return new Promise<string>((resolve, reject) => {
			...
			rpc.request({ type: "prompt", message: prompt }).then((response: RpcResponse) => {
	...
	ipcMain.handle(
		...
		async (_event, projectId: string) => {
			...
			await execFileAsync("git", ["commit", "--allow-empty", "-m", "Initial commit"], {
				cwd: project.path,
				env: { ...process.env, GIT_AUTHOR_NAME: "Snacode", GIT_AUTHOR_EMAIL: "snacode@local", GIT_COMMITTER_NAME: "Snacode", GIT_COMMITTER_EMAIL: "snacode@local" },
			});
		},
	);
	...
	ipcMain.handle(ipcChannels.skillsCreate, async (_event, input: CreateSkillInput) => {
	...
	ipcMain.handle(ipcChannels.promptsCreate, async (_event, input: CreatePromptTemplateInput) => {
	...
	ipcMain.handle(ipcChannels.promptsCreateInProject, async (_event, projectPath: string, input: CreatePromptTemplateInput) => {
	...
	/** 从 prompts.chat 导入 prompt 到本地 ~/.sd/agent/prompts/ */
	ipcMain.handle(ipcChannels.promptStoreImport, async (_event, {
		...
		try {
			...
			const tryCreate = async (tryName: string): Promise<PromptTemplateSummary> => {
				try {
	...
	// GitHub MCP 扩展搜索
	ipcMain.handle(ipcChannels.githubSearch, async (_event, query: string, sortBy: "stars" | "updated" | "forks") => {
		return githubExtensionService.searchExtensions(query, sortBy);
	});
	...
	ipcMain.handle(ipcChannels.extensionsToggle, async (_event, source: string, enabled: boolean) => {
		if (source.startsWith("snacode-") && source.endsWith(".ts")) {
			if (enabled) {
	...
	ipcMain.handle(ipcChannels.agentsPrompt, async (_event, input: SendPromptInput) => {
		...
		const buildFeishuActionInstruction = (chatId?: string) => [
			"当前会话已连接飞书聊天。严禁调用 lark-cli、飞书 IM API 或搜索群聊来发送文件；不要询问 chat_id。需要把本地文件发到当前飞书聊天时，最终回答末尾独立一行写 [SEND_FILE:本地文件路径]，Snacode 会按当前会话绑定自动上传。",
		...
		if (bridgeConnected && docTitle && !hasFeishuBinding) {
			...
			if (tab) {
				...
				void bridge.forwardUserMessageToFeishu(tab.id, input.message).catch((e) => {
					console.error("[Feishu] forward Snacode message failed:", e);
				});
...
app.whenReady().then(async () => {
	// SdDeck → Snacode 数据迁移
		...
		if (result.success) {
			...
			console.error(`[Snacode] Migration failed: ${result.error}`);
		}
	}
...
async function runPostWindowStartupTasks(): Promise<void> {
	...
	const deployExtensionsTo = async (homeDir: string) => {
		...
		for (const extensionName of ["snacode-ask-question.ts", "snacode-nul-redirect-fix.ts", "snacode-plan-mode.ts", "snacode-todo.ts"]) {
			if (disabledBuiltIn.has(extensionName)) continue;
			await ensureSnacodeExtension(extensionName, homeDir).catch((error) => {
	...
	if (activeWslEnvironment) {
		void deployExtensionsTo(activeWslEnvironment.windowsHome).catch(() => {
			console.warn("[Snacode] Failed to deploy extensions to WSL, skipping");
		});
	}
	...
	// 清理已废弃的 snacode-file-capture 扩展：该扩展的功能已被 renderer 端的直接工具参数解析取代。
	void removeStaleSnacodeExtension("snacode-file-capture.ts").catch((error) => {
		console.error("Failed to remove stale snacode-file-capture extension:", error);
	});
...
async function ensureSnacodeExtension(extensionName: string, wslHome?: string): Promise<void> {
	...
	if (!sourceContent) {
		console.warn(`[Snacode] Extension source not found: ${sourcePath}`);
		return;
	}
	...
	console.log(`[Snacode] Installed extension: ${targetPath}`);
}
