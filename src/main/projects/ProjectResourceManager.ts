...
export class ProjectResourceManager {
	...
	async createSkill(input: CreateProjectSkillInput): Promise<SkillSummary> {
	...
	async toggleSkill(projectId: string, skillPath: string, enabled: boolean): Promise<SkillSummary> {
		const project = this.requireProject(projectId);
		const skill = await this.findSkill(project, skillPath);
	...
	private async listSkills(project: Project): Promise<SkillSummary[]> {
	...
	private async scanSkillLocation(location: SkillLocation): Promise<SkillSummary[]> {
	...
	private async collectDirectorySkills(dir: string, location: SkillLocation, out: SkillSummary[]) {
	...
	private toExtensionSummary(name: string, path: string): ExtensionSummary {
		return {
			id: `project:${path}`,
			source: name,
			path,
	...
	async renameSkill(projectId: string, skillPath: string, newName: string): Promise<SkillSummary> {
		const project = this.requireProject(projectId);
		const skill = await this.findSkill(project, skillPath);
