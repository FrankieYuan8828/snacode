...
export class WorktreeService {
	...
	async remove(worktreePath: string, projectPath: string): Promise<boolean> {
		...
		// 删除 Snacode 创建的分支：旧版本使用 Snacode/{slug}，新版本使用与目录名一致的 {slug}。
		// 对外部 worktree 尽量保守，只在“分支名等于目录名”时认为是 Snacode 创建的同名工作区。
		const worktreeDirName = basename(worktreePath);
		if (branch?.startsWith("Snacode/") || branch === worktreeDirName) {
