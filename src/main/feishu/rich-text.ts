...
function markdownToPost(text: string, language: "zh" | "en") {
	...
	const title = cleanInlineMarkdown(heading?.[1] || firstLine || (language === "en" ? "Sd reply" : "Sd 回复")).slice(0, 120);
...
function extractMarkdownTitle(text: string, language: "zh" | "en") {
	const lines = text.replace(/\r\n/g, "\n").split("\n");
	const titleIndex = lines.findIndex((line) => line.trim());
	if (titleIndex < 0) return { title: language === "en" ? "Sd reply" : "Sd 回复", body: text };
