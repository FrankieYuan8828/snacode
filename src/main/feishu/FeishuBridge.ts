...
export class FeishuBridge {
	...
	/** 飞书消息触发中的运行，agent_end 期间不要再走 Snacode 本地同步，避免文件/文本重复发送。 */
	...
	private async handleCommand(ctx: FeishuMessageContext, text: string): Promise<void> {
		...
		switch (command?.toLowerCase()) {
			...
			case "/newdoc": {
				const title = text.split(/\s+/).slice(1).join(" ") || "Snacode Agent 文档";
			...
			case "/whoami":
				await this.sendSmartMessage(chatId,
					`你的 open_id: \`${userId}\`\n\n📋 你可以将此 ID 填入 Snacode 飞书配置中的「你的 Open ID」字段，以便新建会话时自动拉你进群。`
				);
	...
	private handleAgentEvent(agentId: string, event: unknown): void {
		...
		// 只有用户显式手动连接过的 Snacode 会话，才把 Agent 结果同步到飞书。
	...
	private async syncPiMessageToFeishu(agentId: string, chatId: string): Promise<void> {
		...
		await this.processFeishuActions(chatId, agentId).catch((e) =>
			logErr("[Feishu Bridge] process Snacode Feishu actions failed:", e));
	...
	/** 由 Snacode 宿主按当前会话绑定发送文件，避免 Agent 自己搜索群聊发错 chat。 */
	...
	async forwardUserMessageToFeishu(agentId: string, text: string): Promise<void> {
		...
		// 带上 Snacode 标识，方便在飞书中区分消息来源
		await this.sendSmartMessage(chatId, `💻 **Snacode**:\n${text}`);
	...
	private async createNewSession(ctx: FeishuMessageContext, _title?: string): Promise<void> {
		...
		if (projects.length === 0) { await this.sendSmartMessage(chatId, "❌ 请先在 Snacode 中添加项目（工作区），然后重试。"); return; }
	...
	private async handleModelCommand(ctx: FeishuMessageContext, text: string): Promise<void> {
		...
		if (!models.length) { await this.sendSmartMessage(ctx.chatId, "没有可用模型。请先在 Snacode 中配置模型。"); return; }
