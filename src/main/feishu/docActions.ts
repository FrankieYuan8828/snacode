export const FEISHU_DOC_ACTION_HINT = [
	"\n\n[Snacode 飞书能力]",
	"当前会话已连接飞书，Snacode 主进程已配置可用凭证。需要创建飞书文档时，直接产出正文和动作标记。",
 ...
 * - 去掉 Snacode 内部能力提示与宿主注入说明
 * 只保留用户可见的最终回复。
 */
/** 宿主注入给模型的内部指令边界（不应对用户展示）。 */
export const HOST_INSTRUCTION_START = "[Snacode_HOST_INSTRUCTION]";
export const HOST_INSTRUCTION_END = "[/Snacode_HOST_INSTRUCTION]";
...
export function stripHostInstruction(text: string): string {
	...
	// 新格式：明确边界标记
	next = next.replace(
		/\[Snacode_HOST_INSTRUCTION\][\s\S]*?\[\/Snacode_HOST_INSTRUCTION\]\s*/g,
		"",
	);
...
export function wantsFeishuDoc(text: string): string | undefined {
	...
	return titleMatch?.[1] || "Snacode Agent 文档";
}
