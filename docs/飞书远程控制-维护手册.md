...
| 项目 | 路径 | 角色 |
|------|------|------|
| **Snacode** | `snacode/` | 实际运行的 Electron 桌面应用，飞书功能集成在 `src/main/feishu/` |
| **历史参考实现** | 已归档/可删除 | 早期 Pi CLI 飞书扩展参考，不参与 Snacode 运行 |

> **实际运行的是 Snacode（snacode）**。当前飞书功能由 `src/main/feishu/` 和官方 `@larksuiteoapi/node-sdk` 提供，历史参考实现不再作为运行时依赖。
...
│                              │                                     │
│  ┌───────────────────────────▼──────────────────────────────────┐ │
│  │       Sd RPC 子进程 (sd --mode rpc) × N                      │ │
│  │  每个 Agent Tab = 一个独立 sd 进程                           │ │
│  └──────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────┘
                                  │
...
| 持久化 | JSON 文件（`~/.snacode/feishu*.json`） |
...
├── node_modules/                        # 依赖
└── .sd/agent/extensions/feishu/         # 扩展核心代码 (19 个文件)
...
│   │   ├── agent/
│   │   │   └── AgentManager.ts          # Sd RPC 进程管理（create/sendPrompt/stop/list）
│   │   └── ...
│   ├── renderer/src/
│   │   ├── components/feishu/
...
1. Snacode 中创建新 Agent 会话（`index.ts` 中的 `agentsCreate` IPC handler → `feishu.ensureSessionMirror()`）
...
1. 配置中的 `defaultUserOpenId`（用户在 Snacode UI 中填入）
...
| 文件 | 路径 | 内容 |
|------|------|------|
| 多 Bot 配置 | `~/.snacode/feishu.json` | 所有 Bot 的 App ID、加密 Secret、名称、设置 |
| 绑定数据 | `~/.snacode/feishu-bindings-{botId}.json` | 飞书聊天 ↔ Pi 会话的映射关系 |
...
| 文件 | 路径 | 内容 |
|------|------|------|
| 配置 | `~/.sd/agent/feishu/config.json` | App ID、Secret、domain、groupPolicy |
| 状态 | `~/.sd/agent/feishu/state.json` | 飞书会话 ↔ Sd 会话映射 + 模型选择 |
| 桥接路由 | `~/.sd/agent/feishu/bridge.json` | 定时任务路由信息 |
| 调试日志 | `~/.sd/agent/feishu/debug.log` | 最近调试日志 |
| 守护进程日志 | `~/.sd/agent/feishu/daemon.log` | 后台守护进程日志 |
| 去重数据 | `~/.sd/agent/feishu/dedupe.json` | 消息去重持久化 |
| 网关锁 | `~/.sd/agent/locks.json` | 多进程互斥锁 |
...
| `SD_BIN` | sd 可执行文件路径 | `sd` |
| `SD_FEISHU_DAEMON` | 守护进程模式标识 | 未设置 |
...
```bash
cd "/Users/1900th/Downloads/同步空间/Pi Agent/飞书远程控制/snacode"
...
```bash
# 完全清理飞书配置
rm -f ~/.snacode/feishu.json
rm -f ~/.snacode/feishu-bindings-*.json

# 清理历史参考实现残留（如果安装过早期扩展）
rm -rf ~/.sd/agent/feishu/
rm -f ~/.sd/agent/locks.json
```
...
| # | 检查项 | 怎么做 |
|---|--------|--------|
| 1 | Snacode 是否运行 | 确认窗口打开，终端无报错 |
| 2 | 飞书连接状态 | Snacode 侧边栏状态灯 → 应为绿色 |
...
重置后旧 Secret 立即失效，需要更新 Snacode 配置。
...
| Snacode 源码 | https://github.com/1900EasonJin/snacode |
| 历史参考实现 | https://github.com/AX1202/pi-feishu-lark |
...
| 历史参考实现分析 | `4.pi-feishu-lark分析.md` | 早期参考项目源码分析 |
| 开发进度 | `3.开发进度.md` | 功能开发状态 |
| 编译与启动 | `编译与启动指南.md` | 基础启动指南 |
| 机器人绑定教程 | `7.飞书机器人绑定完整教程.md` | 用户侧绑定教程 |
| Snacode 集成方案 | `2.Snacode集成飞书远程控制-完整方案.md` | 集成架构设计 |
...
| HTML 预览 | `预览-Snacode集成方案.html` | 方案可视化预览 |
